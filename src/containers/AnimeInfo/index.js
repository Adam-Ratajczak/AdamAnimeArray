...
import { GetAnime, GetAnimeDemographics, GetAnimeGenres, GetAnimeProducers, GetAnimeThemes, GetAnimeType, GetEpisodes, GetFilterEntry, FilterAnimes, GetAnimeStudios, GetAnimeRelations, GetRelations } from "../../db_module"
      ...
      .then((result) => {
        ...
        for (let rel of result) {
          if (arr.has(rel.RelationID)) {
            arr.get(rel.RelationID).push((<a class="RelationEntry" href={"/anime/" + rel.OtherID.toString()}>{rel.OtherName}</a>))
          } else {
            arr.set(rel.RelationID, [(<a class="RelationEntry" href={"/anime/" + rel.OtherID.toString()}>{rel.OtherName}</a>)])
          }
        }
