...
function LoginBtn() {
  let btns = 0;
  // FIXME: Use loader ???
  // FIXME: Info not used for anything
  const [info, setInfo] = useState(false);
  console.log(LoginMan.UserID());

  useEffect(() => {
    async function f() {
      const response = await LoginMan.userinfo();
      setInfo(response.status === 202 ? await response.json() : false);
    };
    f();
  });
