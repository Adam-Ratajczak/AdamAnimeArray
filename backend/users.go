...
func UserWatched(c echo.Context) error {
	...
	rows, err := db.Query("WITH cte AS ( SELECT AnimeID, EpisodeNr, EpisodeID, (SELECT p.EntryID FROM UserAnimeProgress p WHERE p.UserID = ? AND p.EpisodeID = e1.EpisodeID LIMIT 1) AS 'EntryID', (SELECT p.Progress FROM UserAnimeProgress p WHERE p.UserID = ? AND p.EpisodeID = e1.EpisodeID LIMIT 1) AS 'AnimeProgress' FROM Episodes e1 WHERE (SELECT COUNT(e2.EpisodeID) FROM UserAnimeProgress p2 JOIN Episodes e2 ON p2.EpisodeID = e2.EpisodeID WHERE p2.UserID = ? AND p2.Progress = 2 AND e1.AnimeID = e2.AnimeID) > 0 HAVING AnimeProgress = 1 OR AnimeProgress IS NULL ORDER BY e1.AnimeID, e1.EpisodeNr) SELECT AnimeID, EpisodeNr, (SELECT ap.Seen FROM UserAnimeProgress ap WHERE (SELECT ep.AnimeID FROM Episodes ep WHERE ap.EpisodeID = ep.EpisodeID) = c.AnimeID AND ap.UserID = ? ORDER BY ap.EpisodeID DESC LIMIT 1) AS 'Seen' FROM cte c LEFT JOIN UserAnimeProgress p ON c.EpisodeID = p.EpisodeID WHERE p.EntryID = c.EntryID OR c.EntryID IS NULL GROUP BY AnimeID ORDER BY Seen DESC;", id, id, id, id)
	animes := []AnimeWached{}
	for rows.Next() {
		Watched := AnimeWached{}
		AnimeID := 0
