...
func main() {
	...
	auth := e.Group("/auth")
	{
		...
		auth.POST("/report", GetUserReported)
		auth.POST("/report/player", UserReportPlayer)
		auth.POST("/watchlist", UserWatchlist)
		auth.POST("/watchlist/add", UserWatchlistAdd)
		auth.POST("/watchlist/remove", UserWatchlistRem)
		auth.POST("/history", UserHistory)
		auth.POST("/history/write", UserHistoryWrite)
		auth.POST("/history/clear", UserHistoryClear)
		auth.POST("/comment/write", UserCommentAnime)
		auth.POST("/comment/remove", UserCommentDel)
		auth.POST("/comment/react", UserCommentReact)
		auth.POST("/comment/getreaction", UserCommentGetReactions)
